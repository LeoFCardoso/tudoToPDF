...
public class PDFConverterFactory {
	...
	public static PDFConverter createPDFConverter(String contentType, String strategy) {
		...
		if ("JOD".equals(strategy)
				&& Arrays.asList(new String[] { DOC, XLS, PPT, DOCX, PPTX, XLSX, ODT, ODS, ODP }).contains(contentType)) {
		...
		if ("OfficeToPDF".equals(strategy)
				&& Arrays.asList(new String[] { DOC, XLS, PPT, DOCX, XLSX, PPTX }).contains(contentType)) {
			return new OfficeToPDFConverter();
		}

		if ("AsposeWords".equals(strategy) && Arrays.asList(new String[] { DOC, DOCX, ODT }).contains(contentType)) {
